...
static void moduleEditComponent(Inventory& inv) {
    ...
    if (!c) { std::cout << CLR_RED "  Not found.\n" CLR_RESET; return; }
    c->display();
    std::cout << "\n  1=Name  2=Quantity  3=MinStock  4=UnitPrice  "
                 "5=Category  6=Datasheet  7=Package  8=PinCount\n";
    int ch = inputInt("Field to edit", 0);
    if (ch == 1) c->setName(inputLine("New name: "));
    else if (ch == 2) c->setQuantity(inputInt("New quantity"));
    else if (ch == 3) c->setMinStock(inputInt("New min stock"));
    else if (ch == 4) c->setUnitPrice(inputDouble("New unit price (€)"));
    else if (ch == 5) c->setCategoryPath(inputLine("New category path: "));
    else if (ch == 6) c->setDatasheetPath(inputLine("New datasheet path/URL: "));
    else if (ch == 7) c->setPackageType(inputLine("New package type: "));
    else if (ch == 8) c->setPinCount(inputInt("New pin count"));
...
static void menuInventory(Inventory& inv) {
    while (true) {
        ...
        std::cout
            ...
            << "  2  View component detail\n"
            << "  3  Add component\n"
            << "  4  Edit component\n"
            << "  5  Delete component\n"
            << "  6  List by category\n"
            << "  7  Quick name search\n"
            << "  8  List by package type\n"
            << "  9  List by pin count\n"
            << "  0  Back\n";
        ...
        if (ch == 1) {
            ...
            Component* c = inv.getComponent(id);
            if (c) c->display(); else std::cout << CLR_RED "  Not found.\n" CLR_RESET;
            pause();
        } else if (ch == 3) {
...
static void menuSearch(Inventory& inv) {
    ...
    while (true) {
        ...
        if (ch == 1) {
            ...
            if (!pc_s.empty()) {
                try { sc.setPinCount(std::stoi(pc_s)); } catch(...) {}
            }
            ...
            if (!mp.empty()) {
                try { sc.setMaxUnitPrice(std::stod(mp)); } catch(...) {}
            }
...
int main() {
    ...
    std::cout
        << "\n  Welcome to ECIMS!\n"
        << "  1  Load saved data from '" << DATA_DIR << "/'\n"
        << "  2  Load sample data (demonstration)\n"
        << "  3  Start empty\n";
    int boot = inputInt("Choice", 1);
    if (boot == 1) {
    ...
    while (true) {
        ...
        banner();
        std::cout << CLR_BOLD
            << "\n  Inventory: " << inv.size() << " components"
            << "    Low stock: " << inv.getLowStock().size() << "\n" CLR_RESET
            << "\n"
            << "  [1]  Inventory (CRUD)          [6]  Datasheet Manager\n"
            << "  [2]  Category Browser          [7]  Low Stock Alerts\n"
            << "  [3]  BOM Import & Cross-Ref    [8]  Save to disk\n"
            << "  [4]  Parametric Search         [9]  Load from disk\n"
            << "  [5]  Project Management        [0]  Exit\n";

        int ch = inputInt("\n  Choice", 0);

        if      (ch == 0) break;
        else if (ch == 1) menuInventory(inv);
        else if (ch == 2) menuTaxonomy(inv);
        else if (ch == 3) menuBOM(inv);
        else if (ch == 4) menuSearch(inv);
        else if (ch == 5) menuProjects(inv, pm);
        else if (ch == 6) menuDatasheet(inv);
        else if (ch == 7) menuLowStock(inv);
        else if (ch == 8) {
