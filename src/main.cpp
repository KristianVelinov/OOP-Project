...
static void loadSettings()
{
    ...
    while (std::getline(f, line))
    {
        ...
        {
            if (key == "saveMode")
                g_settings.saveMode = (SaveMode)std::stoi(val);
            else if (key == "confirmDelete")
                g_settings.confirmDelete = (val == "1");
            else if (key == "valueAwareBOM")
                g_settings.valueAwareBOM = (val == "1");
            else if (key == "bomTypeWarnings")
                g_settings.bomTypeWarnings = (val == "1");
            else if (key == "colorTableRows")
                g_settings.colorTableRows = (val == "1");
            else if (key == "dataDir" && !val.empty())
                g_dataDir = val;
        }
        catch (...)
        {
...
static void printCompList(const std::vector<Component *> &list)
{
    ...
    for (int i = 0; i < (int)list.size(); ++i)
    {
        ...
        if (g_settings.colorTableRows && (i % 2 == 1))
            std::cout << CLR_RESET;
        if (low)
            std::cout << CLR_RED " [LOW]" CLR_RESET;
        std::cout << "\n";
    }
...
static void moduleAddComponent(Inventory &inv, ProjectManager &pm)
{
    ...
    int t = arrowMenu("Component type", {"Resistor", "Capacitor", "Inductor", "MOSFET Transistor", "BJT Transistor", "Diode", "Integrated Circuit (IC)", "Connector", "Arbitrary / Other", "Cancel"}, 4);
...
static void moduleEditComponent(Inventory &inv, ProjectManager &pm)
{
    ...
    int ch = arrowMenu("Field to edit", {"Name", "Quantity", "Min stock threshold", "Unit price", "Category path", "Datasheet path / URL", "Package type", "Pin count", "Cancel"}, 4);
...
static void moduleDeleteComponent(Inventory &inv, ProjectManager &pm)
{
    ...
    {
        int ans = arrowMenu("Confirm deletion", {"Yes — delete permanently", "No  — cancel"}, 4);
        if (ans != 0)
        {
            std::cout << "  Cancelled.\n";
            pause();
...
static void menuInventory(Inventory &inv, ProjectManager &pm)
{
    while (true)
    {
        ...
        {
            moduleAddComponent(inv, pm);
        }
        else if (ch == 3)
        {
            moduleEditComponent(inv, pm);
        }
        else if (ch == 4)
        {
            moduleDeleteComponent(inv, pm);
        }
        else if (ch == 5)
        {
            clearScreen();
            banner();
            hdr("List by Category");
...
static void menuBOM(Inventory &inv)
{
    while (true)
    {
        int ch = arrowMenu("BOM Import & Cross-Reference", {"Import KiCad BOM CSV and cross-reference", "Back"}, 4);
...
static void menuSearch(Inventory &inv)
{
    ...
    while (true)
    {
        int ch = arrowMenu("Parametric Search", {"Quick name / ID search", "Filter by type", "Filter by package type", "Filter by exact pin count", "Filter by pin count range", "Advanced: numeric parameter range", "Compound search (combine filters)", "Back"}, 4);
...
static void menuProjects(Inventory &inv, ProjectManager &pm)
{
    // Static state remembers the last opened project between menu visits
    static std::string activeProject = "";
    ...
    while (true)
    {
        ...
        if (activeProject.empty())
        {
            ...
            // Build the dynamic menu list
            std::vector<std::string> opts = {
                CLR_GREEN "[ Create New Project ]" CLR_RESET,
                CLR_YELLOW "[ Restock Component (Global action) ]" CLR_RESET};
            for (const auto &p : projects)
            {
                opts.push_back("Project: " + p);
            }
            opts.push_back("Back to Main Menu");

            int ch = arrowMenu("Project Management", opts, 4);

            if (ch == (int)opts.size() - 1)
            ...
            { // Create New
                ...
                if (!name.empty() && !pm.hasProject(name))
                {
                    pm.createProject(name, desc);
                    std::cout << CLR_GREEN "  ✓ Project '" << name << "' created.\n" CLR_RESET;
                    activeProject = name; // Auto-select the newly created project
                    trySave(inv, pm);
                }
                else
                {
                    std::cout << CLR_RED "  Invalid name or project already exists.\n" CLR_RESET;
                }
                pause();
                continue;
            }
            ...
            { // Restock (Global)
                ...
                {
                    std::cout << CLR_RED "  Failed — component not found.\n" CLR_RESET;
                }
                pause();
                continue;
            }
            else
            {
                ...
                // Offset by 2 to account for the [Create] and [Restock] buttons
                activeProject = projects[ch - 2];
            }
        }
...
static void menuDatasheet(Inventory &inv)
{
    ...
    while (true)
    {
        int ch = arrowMenu("Datasheet Manager", {"Open datasheet for component", "Set / update datasheet path or URL", "Show datasheet path", "Back"}, 4);
...
static void menuLowStock(Inventory &inv)
{
    while (true)
    {
        int ch = arrowMenu("Low Stock Alerts", {"Show low stock report", "Export low stock report to CSV", "Back"}, 4);
...
static void menuSettings()
{
    while (true)
    {
        ...
        auto saveModeStr = [&]() -> std::string
        {
            ...
            {
            case SaveMode::AUTO:
                return CLR_SETTINGON "Auto-save after every change" CLR_RESET;
            case SaveMode::PROMPTED:
                return CLR_SETTINGON "Prompt to save on exit" CLR_RESET;
            default:
                return CLR_SETTINGOFF "Manual (Save to disk menu)" CLR_RESET;
            }
        };
        ...
        int ch = arrowMenu("Settings  [Enter to toggle / change]", {std::string("Save mode           : ") + saveModeStr(), std::string("Confirm before delete: ") + yn(g_settings.confirmDelete), std::string("BOM value-aware match: ") + yn(g_settings.valueAwareBOM), std::string("BOM type warnings    : ") + yn(g_settings.bomTypeWarnings), std::string("Alternating row tint : ") + yn(g_settings.colorTableRows), std::string("Data directory       : ") + CLR_CYAN + g_dataDir + CLR_RESET, "Back"}, 4);
        ...
        case 0: // Cycle save mode
            ...
            case SaveMode::MANUAL:
                g_settings.saveMode = SaveMode::AUTO;
                break;
            case SaveMode::AUTO:
                g_settings.saveMode = SaveMode::PROMPTED;
                break;
            case SaveMode::PROMPTED:
                g_settings.saveMode = SaveMode::MANUAL;
                break;
            }
            break;
        case 1:
            g_settings.confirmDelete = !g_settings.confirmDelete;
            break;
        case 2:
            g_settings.valueAwareBOM = !g_settings.valueAwareBOM;
            break;
        case 3:
            g_settings.bomTypeWarnings = !g_settings.bomTypeWarnings;
            break;
        case 4:
            g_settings.colorTableRows = !g_settings.colorTableRows;
            break;
        case 5:
        {
            hdr("Data Directory");
...
int main()
{
    ...
    while (true)
    {
        auto saveModeTag = [&]() -> std::string
        {
            ...
            {
            case SaveMode::AUTO:
                return CLR_GREEN " [auto]" CLR_RESET;
            case SaveMode::PROMPTED:
                return CLR_YELLOW " [prompted]" CLR_RESET;
            default:
                return CLR_DIM " [manual]" CLR_RESET;
            }
        };
        ...
        else if (ch == 10)
        {
            ...
            {
                int ans = arrowMenu("Save before exiting?", {"Yes — save and exit", "No  — exit without saving", "Cancel — go back"}, 4);
                if (ans == 0)
                {
